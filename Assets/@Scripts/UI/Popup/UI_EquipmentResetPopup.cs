...
public class UI_EquipmentResetPopup : UI_Popup
{
  ...
  private void Refresh()
  {
    ...
    if (_equipment.equipmentData.equipmentGrade == EEquipmentGrade.Common)
      GetObject((int)GameObjects.ToggleGroupObject).SetActive(false);
    else if (_equipment.equipmentData.equipmentGrade == EEquipmentGrade.Uncommon)
      GetObject((int)GameObjects.ToggleGroupObject).SetActive(false);
    else if(_equipment.equipmentData.equipmentGrade == EEquipmentGrade.Rare)
      GetObject((int)GameObjects.ToggleGroupObject).SetActive(false);
    else if(_equipment.equipmentData.equipmentGrade == EEquipmentGrade.Epic)
      GetObject((int)GameObjects.ToggleGroupObject).SetActive(false);
    else if(_equipment.equipmentData.equipmentGrade == EEquipmentGrade.Legendary)
      GetObject((int)GameObjects.ToggleGroupObject).SetActive(false);
    else
    {
  ...
  private void EquipmentResetRefresh()
  {
    ...
    switch (_equipment.equipmentData.equipmentGrade)
    {
      ...
      case EEquipmentGrade.Legendary3:
        ...
        break;
      case EEquipmentGrade.Myth:
      case EEquipmentGrade.Myth1:
      case EEquipmentGrade.Myth2:
      case EEquipmentGrade.Myth3:
  ...
  private void OnClickEquipmentResetButton()
  {
    ...
    Managers.Game.ExchangeMaterial(Managers.Data.MaterialDic[ID_GOLD], gold);
  ...
  private void OnClickEquipmentDowngradeButton()
  {
    if (_equipment.equipmentData.downgradeEquipmentCode == null) return;
    
    if (Managers.Data.EquipDataDic.TryGetValue(_equipment.equipmentData.downgradeEquipmentCode, out Data.EquipmentData downgradedEquip) == false) return;
  ...
  {
    // 활성화 후 토글 클릭 방지
    if (_isSelectedResetTapTap) return;
  ...
  private void OnClickDowngradeTapToggle()
  {
    // 활성화 후 토글 클릭 방지
    if (_isSelectedDowngradeTapTap == true) return;
