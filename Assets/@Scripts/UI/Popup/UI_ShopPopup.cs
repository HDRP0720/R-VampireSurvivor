...
public class UI_ShopPopup : UI_Popup
{
  ...
  // Property
  private UI_GachaListPopup GachaListPopupUI
  {
    get
    {
  ...
  private void Update()
  {
    ...
    else if (Input.GetKeyDown(KeyCode.F2))
    {
      ...
      Managers.Game.AddMaterialItem(ID_RING_SCROLL, 10);
      Managers.Game.AddMaterialItem(ID_BELT_SCROLL, 10);
      Managers.Game.AddMaterialItem(ID_ARMOR_SCROLL, 10);
      Managers.Game.AddMaterialItem(ID_BOOTS_SCROLL, 10);

      Refresh();
    }
    else if (Input.GetKeyDown(KeyCode.F3))
    {
      Managers.Game.ExchangeMaterial(Managers.Data.MaterialDic[ID_STAMINA], 10);
      Refresh();
    }
  }
#endif
  ...
  private void Refresh()
  {
    ...
    #region For dealing with Refresh bugs
  ...
  private void DoGacha(EGachaType gachaType, int count = 1)
  {
    List<Equipment> res = new List<Equipment>();
    res = Managers.Game.DoGacha(gachaType, count).ToList();
    if (Managers.Game.DicMission.TryGetValue(EMissionTarget.GachaOpen, out MissionInfo mission))
      mission.progress++; 
    Managers.UI.ShowPopupUI<UI_GachaResultsPopup>().SetInfo(res);
  }
  
  #region 패키지 버튼 (보류)
  // private void OnClickStagePackageButton() // 챕터 패키지
  // {
  //     // 상품 구매 모듈 호출
  // }
  // private void OnClickStagePackagePrevButton() // 이전 챕터 버튼
  // {
  //     // 패키지의 구성을 이전 패키지 구성으로 변경
  // }
  // private void OnClickStagePackageNextButton() // 다음 챕터 버튼
  // {
  //     // 패키지의 구성을 다음 패키지 구성으로 변경
  // }
  // private void OnClickBeginnerPackageButton() // 비기너 패키지
  // {
  //     // 상품 구매 모듈 호출
  // }
  #endregion
  
  #region 가챠 상점 버튼 (일부 보류)
  // private void OnClickGachaListButton() // 픽업 가챠 확률표 호출 버튼
  // {
  //   GachaListPopupUI.gameObject.SetActive(true);
  //   GachaListPopupUI.SetInfo(EGachaType.PickupGacha);
  // }
  // private void OnClickPickupGachaInfoButton() // 픽업 가챠 정보 버튼
  // {
  //   Managers.UI.ShowPopupUI<UI_PickupGachaInfoPopup>();
  // }
  // private void OnClickOpenOneButton() // 픽업 가챠 1회 열기
  // {
  //   Managers.Game.Dia -= 300;
  //   DoGacha(EGachaType.PickupGacha, 1);
  // }
  // private void OnClickOpenTenButton() // 픽업 가챠 10회 열기
  // {
  //   Managers.Game.Dia -= 2750;
  //   DoGacha(EGachaType.PickupGacha, 10);
  // }
  private void OnClickCommonGachaOpenButton() // 일반 가챠 오픈 버튼
  {
    Managers.Sound.PlayButtonClick();
    if (Managers.Game.ItemDictionary.TryGetValue(ID_SILVER_KEY, out int keyCount))
    {
      if (keyCount > 0) 
      { 
        Managers.Game.RemoveMaterialItem(ID_SILVER_KEY, 1);
        DoGacha(EGachaType.CommonGacha, 1);
        Refresh();
      }
      else
      {
  ...
  private void OnClickADCommonGachaOpenButton() // 일반 가챠 광고 오픈 버튼
  {
    Managers.Sound.PlayButtonClick();
  ...
  private void OnClickADAdvancedGachaOpenButton() // 고급 가챠 광고 오픈 버튼
  {
    Managers.Sound.PlayButtonClick();
  ...
  private void OnClickCommonGachaListButton() // 일반 가챠 리스트 버튼
  {
    Managers.Sound.PlayButtonClick();
    GachaListPopupUI.SetInfo(EGachaType.CommonGacha);
  }
  private void OnClickAdvancedGachaOpenButton() // 고급 가챠 오픈 버튼
  {
    Managers.Sound.PlayButtonClick();
    if (Managers.Game.ItemDictionary.TryGetValue(ID_GOLD_KEY, out int keyCount))
    {
      if (keyCount > 0)
      {
        Managers.Game.RemoveMaterialItem(ID_GOLD_KEY, 1);
        DoGacha(EGachaType.AdvancedGacha, 1);
        Refresh();
      }
      else
      {
  ...
  private void OnClickAdvancedGachaOpenTenButton() // 고급 가챠 10회 버튼
  {
    Managers.Sound.PlayButtonClick();
    if (Managers.Game.ItemDictionary.TryGetValue(ID_GOLD_KEY, out int keyCount))
    {
      if (keyCount >= 10)
      {
        Managers.Game.RemoveMaterialItem(ID_GOLD_KEY, 10);
        DoGacha(EGachaType.AdvancedGacha, 10);
        Refresh();
      }
      else
      {
  ...
  private void OnClickAdvancedGachaListButton() // 고급 가챠 리스트 버튼
  {
    Managers.Sound.PlayButtonClick();
    GachaListPopupUI.SetInfo(EGachaType.AdvancedGacha);
  }
  #endregion
  
  #region 다이아 상점 버튼 (보류)
  // private void OnClickFreeDiaButton() // 무료 다이아 버튼
  // {
  ...
  // }
  // private void OnClickFirstDiaProductButton() // 첫번째 다이아 구매 버튼
  // {
  //   // 구매 모듈 호출
  // }
  // private void OnClickSecondDiaProductButton() // 두번째 다이아 구매 버튼
  // {
  //   // 구매 모듈 호출
  // }
  // private void OnClickThirdDiaProductButton() // 세번째 다이아 구매 버튼
  // {
  //   // 구매 모듈 호출
  // }
  // private void OnClickFourthDiaProductButton() // 네번째 다이아 구매 버튼
  // {
  //   // 구매 모듈 호출
  // }
  // private void OnClickFifthDiaProductButton() // 다섯번째 다이아 구매 버튼
  // {
  //   // 구매 모듈 호출
  // }
  #endregion
  ...
  private void OnClickAdKeyButton() // 광고 열쇠 버튼
  {
    ...
    if (Managers.Game.BronzeKeyCountAds > 0)
    {
      ...
      //   int[] count = new int[1];
      //
      //   spriteName[0] = Managers.Data.MaterialDic[ID_BRONZE_KEY].spriteName;
      //   count[0] = 1;
      //
  ...
  private void OnClickGoldKeyProductButton() // 골드 열쇠 구매 버튼
  {
    Managers.Sound.PlayButtonClick();
  ...
  #region 골드 상점 버튼
  private void OnClickFreeGoldButton() // 무료 골드 버튼
  {
    Managers.Sound.PlayButtonClick();
  ...
  private void OnClickFirstGoldProductButton() // 첫번째 골드 구매 버튼
  {
    Managers.Sound.PlayButtonClick();
    if (Managers.Game.Dia >= 300)
    {
  ...
  private void OnClickSecondGoldProductButton() // 두번째 골드 구매 버튼
  {
    Managers.Sound.PlayButtonClick();
